...
const numbers : number[] = [17, 21, 16, 14, 9, 22, 12, 43, 99, 44, 32];

const a : number[] = numbers.map((number) => number + 1);
const b : number[] = numbers.filter((number) => number % 2 != 0);
const c : number[] = numbers.map((number) => {
    if(number % 2 != 0){
        return number += 1
    } else {
        return number
...
const d : number[] = numbers.filter((number) => number < 20);
const e : number[] = numbers.filter((number) => number < 17).map((number) => number + 1);
const f : number = numbers.filter((number) => number < 20).reduce((previous, next) => previous + next, 0);
